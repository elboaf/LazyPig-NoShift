...
BINDING_HEADER_LP_HEADER = "LazyPig-NoShift";
...
BINDING_NAME_MENU = "LazyPig-NoShift Menu";
...
function LazyPig_OnEvent(event)
	if (event == "ADDON_LOADED") and (arg1 == "LazyPig-NoShift") then
		local LP_TITLE = GetAddOnMetadata("LazyPig-NoShift", "Title")
		local LP_VERSION = GetAddOnMetadata("LazyPig-NoShift", "Version")
		local LP_AUTHOR = GetAddOnMetadata("LazyPig-NoShift", "Author")
	...
	--	this:UnregisterEvent("PLAYER_ENTERING_WORLD")
		...
		if LPNCONFIG.CAM then SetCVar("cameraDistanceMax",50) end
		if LPNCONFIG.LOOT then UIPanelWindows["LootFrame"] = nil end
		QuestRecord["index"] = 0
	...
	elseif (LPNCONFIG.SALVA and (event == "PLAYER_AURAS_CHANGED" or event == "UPDATE_BONUS_ACTIONBAR" and LazyPig_PlayerClass("Druid", "player") or event == "UNIT_INVENTORY_CHANGED")) then
		LazyPig_CheckSalvation()
	elseif (LPNCONFIG.REMOVEMANABUFFS and (event == "PLAYER_AURAS_CHANGED" or event == "UPDATE_BONUS_ACTIONBAR" and LazyPig_PlayerClass("Druid", "player") or event == "UNIT_INVENTORY_CHANGED"))then
		LazyPig_CheckManaBuffs()
		
	elseif(event == "DUEL_REQUESTED") then
		duel_active = true
		if LPNCONFIG.DUEL and not IsShiftKeyDown() then --dnd_active and
			duel_active = nil
			CancelDuel()
	...
	elseif(event == "PLAYER_DEAD") then
		if LPNCONFIG.RBG and LazyPig_BG() then
			RepopMe();
		end	
	...
	elseif(event == "UI_ERROR_MESSAGE") then
		if(string.find(arg1, "mounted") or string.find(arg1, "while silenced")) and LPNCONFIG.DISMOUNT then
			UIErrorsFrame:Clear()
			LazyPig_Dismount()
		end
	...
	elseif (event == "CHAT_MSG_SYSTEM") then
		...
		elseif(string.find(arg1, string.sub(MARKED_DND, 1, string.len(MARKED_DND) -3))) then
			afk_active = false
			dnd_active = true
			--if LPNCONFIG.DUEL then CancelDuel() UIErrorsFrame:AddMessage("Duel Decline Atctive - DND") end
		
		elseif(string.find(arg1, string.sub(MARKED_AFK, 1, string.len(MARKED_AFK) -2))) then
			afk_active = true
			if LPNCONFIG.EBG and not LazyPig_Raid() and not LazyPig_Dungeon() then UIErrorsFrame:AddMessage("Auto Join BG Inactive - AFK") end

		elseif LPNCONFIG.AQUE and string.find(arg1 ,"Queued") and UnitIsPartyLeader("player") then
	...
	elseif(event == "GOSSIP_SHOW") then
		...
		local processgossip = IsShiftKeyDown() or LPNCONFIG.GOSSIP
		...
		for i=1, getn(GossipOptions) do
			...
			elseif (GossipOptions[i] == "trainer" and dsc == "Reset my talents.") then
				gossipbreak = false
			elseif ((GossipOptions[i] == "trainer" and processgossip) or (GossipOptions[i] == "vendor" and processgossip) or (GossipOptions[i] == "battlemaster" and (LPNCONFIG.QBG or processgossip)) or (GossipOptions[i] == "gossip" and processgossip) or (GossipOptions[i] == "petition" and (IsAltKeyDown() or IsShiftKeyDown() or string.find(dsc, "Teleport me to the Molten Core")) and processgossip)) then
				gossipnr = i
	...
	elseif(event == "PARTY_INVITE_REQUEST") then
		local check1 = not LPNCONFIG.DINV or LPNCONFIG.DINV and not LazyPig_BG() and not LazyPig_Queue()
		local check2 = LPNCONFIG.GINV and IsGuildMate(arg1) or LPNCONFIG.FINV and IsFriend(arg1) or not IsGuildMate(arg1) and not IsFriend(arg1) and LPNCONFIG.SINV
		if check1 and check2 then
			AcceptGroupInvite();
		end
	elseif(event == "RESURRECT_REQUEST" and LPNCONFIG.REZ) then
...
function LazyPig_StaticPopup_OnShow()
	if this.which == "QUEST_ACCEPT" and LazyPig_BG() and LPNCONFIG.SBG then
		UIErrorsFrame:Clear();
...
function MailtoCheck(msg) 
	if MailTo_Option then -- to avoid conflicts with mailto addon
		local disable = LPNCONFIG.RIGHT or LPNCONFIG.SHIFT
		MailTo_Option.noshift = disable
		MailTo_Option.noauction = disable 
		MailTo_Option.notrade = disable
		MailTo_Option.noclick = disable
		if msg then DEFAULT_CHAT_FRAME:AddMessage("LazyPig-NoShift: Warning Improved Right Click and Easy Split/Merge features may override MailTo addon functionality !") end
	end
end
...
function LazyPig_AutoJoinBG(index, map_name)	
	local keyenter = IsAltKeyDown() and IsControlKeyDown() and not tradestatus and not mailstatus and not auctionstatus and GetTime() > delayaction and GetTime() > (tradedelay + 0.5)
	if LPNCONFIG.EBG or keyenter then	
...
function LazyPig_AutoLeaveBG()
	...
	if LPNCONFIG.LBG or keyenter then
...
function LazyPig_AutoRoll(id)
	...
	if LPNCONFIG.ZG and zone == "Zul'Gurub" then										 
		if string.find(name, "Hakkari Bijou") or string.find(name, "Coin") then
			cfg = LPNCONFIG.ZG
			RollOnLoot(id, LPNCONFIG.ZG);
		end
	end

	if LPNCONFIG.MC and zone == "Molten Core" then	
		if string.find(name, "Fiery Core") or string.find(name, "Lava Core") or string.find(name, "Blood of the Mountain") then
			cfg = LPNCONFIG.MC
			RollOnLoot(id, LPNCONFIG.MC);
		end	
	end
	
	if LPNCONFIG.AQ and string.find(zone,"Ahn'Qiraj") then	
		if not string.find(name, "Scarab Brooch") and not string.find(name, "Petrified Scarab") then
			if string.find(name, "Scarab") or string.find(name, "Idol") then
				cfg = LPNCONFIG.AQ
				RollOnLoot(id, LPNCONFIG.AQ);
			end
		end
	end
	
	if LPNCONFIG.AQMOUNT and string.find(zone,"Ahn'Qiraj") then	
		if string.find(name, "Blue Qiraji Resonating") or string.find(name, "Green Qiraji Resonating") or string.find(name, "Yellow Qiraji Resonating") then
			cfg = LPNCONFIG.AQMOUNT
			RollOnLoot(id, LPNCONFIG.AQMOUNT);
		end	
	end	
	
	if LPNCONFIG.SAND then	
		if string.find(name ,"Corrupted Sand") then
			cfg = LPNCONFIG.SAND
			RollOnLoot(id, LPNCONFIG.SAND);
		end
	end

	if LPNCONFIG.NAXX and zone == "Naxxramas" then	
		if string.find(name, "Wartorn") or string.find(name, "Thawing") then
			cfg = LPNCONFIG.NAXX
			RollOnLoot(id, LPNCONFIG.NAXX);
		end	
	end

	if LPNCONFIG.BWL and string.find(zone, "Blackwing") then	
		if string.find(name, "Hourglass Sand") or string.find(name, "Elementium Ore") then
			cfg = LPNCONFIG.BWL
			RollOnLoot(id, LPNCONFIG.BWL)
		end	
	end
	...
	if LPNCONFIG.WHITE_TAILORING and quality == 1 and (string.find(name, "loth") or string.find(name, "Silk")) then
		cfg = LPNCONFIG.WHITE_TAILORING
		RollOnLoot(id, LPNCONFIG.WHITE_TAILORING)
	end
	
	-- Config for Food and Drink
	if LPNCONFIG.FOOD_AND_DRINK and quality == 1 then
		...
		if found and itemIdFromLink then
			...
			if itemType == "Consumable" and subType==20 then
				cfg = LPNCONFIG.FOOD_AND_DRINK
				RollOnLoot(id, LPNCONFIG.FOOD_AND_DRINK)
			end
		end
	end

	if LPNCONFIG.ES_SHARDS and string.find(zone, "Emerald Sanctum") then
		if string.find(name, "Dreamscale") or string.find(name, "Fading Dream Fragment") or string.find(name, "Small Dream Shard") then
			cfg = LPNCONFIG.ES_SHARDS
			RollOnLoot(id, LPNCONFIG.ES_SHARDS)
		end
	end

	if LPNCONFIG.ROLLMSG and type(cfg) == "number" then
...
function LazyPig_GreenRoll()
	RollReturn = function()
		local txt = ""
		if LPNCONFIG.GREEN == 1 then
			txt = "NEED"
		elseif LPNCONFIG.GREEN == 2 then
			txt = "GREED"
		elseif LPNCONFIG.GREEN == 0 then
			txt = "PASS"
		end
		return txt
	end
	local pass = nil
	if LPNCONFIG.GREEN then	
...
function LazyPig_UseContainerItem(ParentID,ItemID)
		if LPNCONFIG.SHIFTSPLIT and not CursorHasItem() and not merchantstatus and IsShiftKeyDown() and not IsAltKeyDown() then
		...
		elseif LPNCONFIG.RIGHT and tradestatus and not IsShiftKeyDown() and not IsAltKeyDown() and LazyPig_ItemIsTradeable(ParentID,ItemID) then
		...
		elseif LPNCONFIG.RIGHT and GMailFrame and GMailFrame:IsVisible() and not CursorHasItem() then
			local i
		...
		elseif LPNCONFIG.RIGHT and CT_MailFrame and CT_MailFrame:IsVisible() and not IsShiftKeyDown() and not IsAltKeyDown() then
			local bag, item = ParentID,ItemID
		...
		elseif LPNCONFIG.RIGHT and mailstatus and not IsShiftKeyDown() and not IsAltKeyDown() then
		...
		elseif LPNCONFIG.RIGHT and auctionstatus and not IsShiftKeyDown() and not IsAltKeyDown() then
			if not LazyPig_ItemIsTradeable(ParentID,ItemID) then
...
function LazyPig_RefreshNameplates()
	if LPNCONFIG.EPLATE then
		ShowNameplates();
	elseif LPNCONFIG.HPLATE then
		HideNameplates();
	end
	if LPNCONFIG.FPLATE then
...
function LazyPig_GetOption(num)
	...
	if num == 00 and LPNCONFIG.GREEN == 1
	or num == 01 and LPNCONFIG.GREEN == 2 
	or num == 02 and LPNCONFIG.GREEN == 0 
	or num == 03 and LPNCONFIG.ZG == 1		
	or num == 04 and LPNCONFIG.ZG == 2
	or num == 05 and LPNCONFIG.ZG == 0
	or num == 06 and LPNCONFIG.MC == 1
	or num == 07 and LPNCONFIG.MC == 2
	or num == 08 and LPNCONFIG.MC == 0
	or num == 09 and LPNCONFIG.AQ == 1
	or num == 10 and LPNCONFIG.AQ == 2
	or num == 11 and LPNCONFIG.AQ == 0
	or num == 12 and LPNCONFIG.AQMOUNT == 1
	or num == 13 and LPNCONFIG.AQMOUNT == 2
	or num == 14 and LPNCONFIG.AQMOUNT == 0
	or num == 15 and LPNCONFIG.SAND == 1
	or num == 16 and LPNCONFIG.SAND == 2
	or num == 17 and LPNCONFIG.SAND == 0
	or num == 18 and LPNCONFIG.NAXX == 1
	or num == 19 and LPNCONFIG.NAXX == 2
	or num == 20 and LPNCONFIG.NAXX == 0
	or num == 21 and LPNCONFIG.ROLLMSG
	or num == 22 and LPNCONFIG.WORLDDUNGEON
	or num == 23 and LPNCONFIG.WORLDRAID
	or num == 24 and LPNCONFIG.WORLDBG
	or num == 25 and LPNCONFIG.WORLDUNCHECK
	or num == 26 and LPNCONFIG.BWL == 1
	or num == 27 and LPNCONFIG.BWL == 2
	or num == 28 and LPNCONFIG.BWL == 0
	or num == 30 and LPNCONFIG.GINV
	or num == 31 and LPNCONFIG.FINV
	or num == 32 and LPNCONFIG.SINV
	or num == 33 and LPNCONFIG.DINV
	or num == 40 and LPNCONFIG.FPLATE
	or num == 41 and LPNCONFIG.EPLATE
	or num == 42 and LPNCONFIG.HPLATE 
	or num == 50 and LPNCONFIG.EBG
	or num == 51 and LPNCONFIG.LBG
	or num == 52 and LPNCONFIG.QBG
	or num == 53 and LPNCONFIG.RBG
	or num == 54 and LPNCONFIG.AQUE
	or num == 55 and LPNCONFIG.SBG
	
	or num == 60 and LPNCONFIG.SALVA == 1
	or num == 61 and LPNCONFIG.SALVA == 2

	or num == 62 and LPNCONFIG.REMOVEMANABUFFS == 1

	or num == 63 and LPNCONFIG.ASPECT

	or num == 90 and LPNCONFIG.SUMM
	
	or num == 70 and LPNCONFIG.SPAM
	or num == 71 and LPNCONFIG.SPAM_UNCOMMON
	or num == 72 and LPNCONFIG.SPAM_RARE
	or num == 73 and LPNCONFIG.SPAM_LOOT
	
	or num == 91 and LPNCONFIG.LOOT
	or num == 92 and LPNCONFIG.RIGHT
	or num == 93 and LPNCONFIG.SHIFTSPLIT
	or num == 94 and LPNCONFIG.CAM
	or num == 95 and LPNCONFIG.SPECIALKEY
	or num == 96 and LPNCONFIG.DUEL		
	or num == 97 and LPNCONFIG.REZ
	or num == 98 and LPNCONFIG.GOSSIP
	or num == 100 and LPNCONFIG.DISMOUNT
	or num == 101 and LPNCONFIG.SPAM
	or num == 102 and LPNCONFIG.WHITE_TAILORING == 1
	or num == 103 and LPNCONFIG.WHITE_TAILORING == 2
	or num == 104 and LPNCONFIG.WHITE_TAILORING == 0
	or num == 105 and LPNCONFIG.FOOD_AND_DRINK == 1
	or num == 106 and LPNCONFIG.FOOD_AND_DRINK == 2
	or num == 107 and LPNCONFIG.FOOD_AND_DRINK == 0
	or num == 108 and LPNCONFIG.ES_SHARDS == 1
	or num == 109 and LPNCONFIG.ES_SHARDS == 2
	or num == 110 and LPNCONFIG.ES_SHARDS == 0
...
function LazyPig_SetOption(num)
	...
	if num == 00 then 
		LPNCONFIG.GREEN = 1
		if not checked then LPNCONFIG.GREEN = nil end
	...
	elseif num == 03 then
		LPNCONFIG.ZG = 1
		if not checked then LPNCONFIG.ZG = nil end
	...
	elseif num == 06 then
		LPNCONFIG.MC = 1
		if not checked then LPNCONFIG.MC = nil end
	...
	elseif num == 09 then
		LPNCONFIG.AQ = 1
		if not checked then LPNCONFIG.AQ = nil end
	...
	elseif num == 12 then
		LPNCONFIG.AQMOUNT = 1
		if not checked then LPNCONFIG.AQMOUNT = nil end
	...
	elseif num == 15 then
		LPNCONFIG.SAND = 1
		if not checked then LPNCONFIG.SAND = nil end
	...
	elseif num == 18 then
		LPNCONFIG.NAXX = 1
		if not checked then LPNCONFIG.NAXX = nil end
		LazyPigMenuObjects[19]:SetChecked(nil)
	...
	elseif num == 20 then
		...
		LazyPigMenuObjects[19]:SetChecked(nil)	
	elseif num == 21 then 
		LPNCONFIG.ROLLMSG = true
		if not checked then LPNCONFIG.ROLLMSG = nil end		
	elseif num == 22 then															
		LPNCONFIG.WORLDDUNGEON = true					--fixed
		if not checked then LPNCONFIG.WORLDDUNGEON = nil end
		if LPNCONFIG.WORLDDUNGEON or LPNCONFIG.WORLDRAID or LPNCONFIG.WORLDBG then 
			LPNCONFIG.WORLDUNCHECK = nil
			LazyPigMenuObjects[25]:SetChecked(nil)
		end
		LazyPig_ZoneCheck()
	elseif num == 23 then 
		LPNCONFIG.WORLDRAID = true
		if not checked then LPNCONFIG.WORLDRAID = nil end
		if LPNCONFIG.WORLDDUNGEON or LPNCONFIG.WORLDRAID or LPNCONFIG.WORLDBG then 
			LPNCONFIG.WORLDUNCHECK = nil
			LazyPigMenuObjects[25]:SetChecked(nil)
		end
		LazyPig_ZoneCheck()
	elseif num == 24 then 
		LPNCONFIG.WORLDBG = true
		if not checked then LPNCONFIG.WORLDBG = nil end
		if LPNCONFIG.WORLDDUNGEON or LPNCONFIG.WORLDRAID or LPNCONFIG.WORLDBG then 
			LPNCONFIG.WORLDUNCHECK = nil
			LazyPigMenuObjects[25]:SetChecked(nil)
		end
		LazyPig_ZoneCheck()
	elseif num == 25 then 
		LPNCONFIG.WORLDUNCHECK = true
		if not checked then 
			LPNCONFIG.WORLDUNCHECK = nil
		else	
			LPNCONFIG.WORLDDUNGEON = nil
			LPNCONFIG.WORLDRAID = nil
			LPNCONFIG.WORLDBG = nil
	...
	elseif num == 26 then
		LPNCONFIG.BWL = 1
		if not checked then LPNCONFIG.BWL = nil end
	...
	elseif num == 30 then 								--fixed
		LPNCONFIG.GINV = true
		if not checked then LPNCONFIG.GINV = nil end
	elseif num == 31 then 
		LPNCONFIG.FINV = true
		if not checked then LPNCONFIG.FINV = nil end
	elseif num == 32 then 
		LPNCONFIG.SINV = true
		if not checked then LPNCONFIG.SINV = nil end
	elseif num == 33 then 
		LPNCONFIG.DINV = true
		if not checked then LPNCONFIG.DINV = nil end	
	elseif num == 40 then 								--fixed
		LPNCONFIG.FPLATE = true
		if not checked then LPNCONFIG.FPLATE = nil end
		if LPNCONFIG.EPLATE and LPNCONFIG.FPLATE then 
			LPNCONFIG.HPLATE = nil 
			LazyPigMenuObjects[42]:SetChecked(nil)
		end
		LazyPig_RefreshNameplates()
	elseif num == 41 then 
		LPNCONFIG.EPLATE = true
		if not checked then LPNCONFIG.EPLATE = nil end
		if LPNCONFIG.EPLATE and LPNCONFIG.FPLATE then 
			LPNCONFIG.HPLATE = nil 
			LazyPigMenuObjects[42]:SetChecked(nil)
		end
	...
	elseif num == 50 then --fixed
		LPNCONFIG.EBG = true
		if not checked then LPNCONFIG.EBG = nil end
	elseif num == 51 then 
		LPNCONFIG.LBG = true
		if not checked then LPNCONFIG.LBG = nil end
	elseif num == 52 then 
		LPNCONFIG.QBG = true
		if not checked then LPNCONFIG.QBG = nil end
	elseif num == 53 then 
		LPNCONFIG.RBG = true
		if not checked then LPNCONFIG.RBG = nil end
	elseif num == 54 then 
		LPNCONFIG.AQUE = true
		if not checked then LPNCONFIG.AQUE = nil end		
	elseif num == 55 then
		LPNCONFIG.SBG  = true
		if not checked then LPNCONFIG.SBG  = nil end	
	elseif num == 60 then
		LPNCONFIG.SALVA = 1
		if not checked then LPNCONFIG.SALVA = nil end
	...
	elseif num == 61 then 
		...
		LazyPig_CheckSalvation()
	elseif num == 62 then
		LPNCONFIG.REMOVEMANABUFFS = 1
		if not checked then LPNCONFIG.REMOVEMANABUFFS = nil end
		LazyPig_CheckManaBuffs()		
	elseif num == 63 then
		LPNCONFIG.ASPECT = true
		if not checked then LPNCONFIG.ASPECT = nil end
		LazyPig_CancelAspect()
	elseif num == 70 then --fixed
		LPNCONFIG.SPAM = true
		if not checked then LPNCONFIG.SPAM = nil end
	elseif num == 71 then 
		LPNCONFIG.SPAM_UNCOMMON = true
		if not checked then LPNCONFIG.SPAM_UNCOMMON = nil end
	elseif num == 72 then 
		LPNCONFIG.SPAM_RARE	 = true
		if not checked then LPNCONFIG.SPAM_RARE	 = nil end	
	elseif num == 73 then 
		LPNCONFIG.SPAM_LOOT	 = true
		if not checked then LPNCONFIG.SPAM_LOOT	 = nil end		
		
	elseif num == 90 then
		LPNCONFIG.SUMM = true
		if not checked then LPNCONFIG.SUMM = nil end	
	elseif num == 91 then 
		LPNCONFIG.LOOT = true
		if not checked then LPNCONFIG.LOOT = nil end
	elseif num == 92 then 
		LPNCONFIG.RIGHT = true
		if not checked then LPNCONFIG.RIGHT = nil end
		MailtoCheck(LPNCONFIG.RIGHT)
	elseif num == 93 then--fixed
		LPNCONFIG.SHIFTSPLIT = true
		if not checked then LPNCONFIG.SHIFTSPLIT = nil end
		MailtoCheck(LPNCONFIG.SHIFTSPLIT)
	elseif num == 94 then--fixed 
		LPNCONFIG.CAM = true
		if not checked then LPNCONFIG.CAM = nil end
		if LPNCONFIG.CAM then SetCVar("cameraDistanceMax",50) else SetCVar("cameraDistanceMaxFactor",1) SetCVar("cameraDistanceMax",15) end	
	elseif num == 95 then 
		LPNCONFIG.SPECIALKEY = true
		if not checked then LPNCONFIG.SPECIALKEY = nil end	
	elseif num == 96 then
		LPNCONFIG.DUEL = true
		if not checked then LPNCONFIG.DUEL = nil end
		if LPNCONFIG.DUEL then CancelDuel() end
	elseif num == 97 then
		LPNCONFIG.REZ = true
		if not checked then LPNCONFIG.REZ = nil end	
	elseif num == 98 then
		LPNCONFIG.GOSSIP = true
		if not checked then LPNCONFIG.GOSSIP = nil end
	elseif num == 100 then
		LPNCONFIG.DISMOUNT = true
		if not checked then LPNCONFIG.DISMOUNT = nil end	
	elseif num == 101 then
		LPNCONFIG.SPAM  = true
		if not checked then LPNCONFIG.SPAM  = nil end			
	elseif num == 102 then
		LPNCONFIG.WHITE_TAILORING = 1
		if not checked then LPNCONFIG.WHITE_TAILORING = nil end
		LazyPigMenuObjects[103]:SetChecked(nil)
		LazyPigMenuObjects[104]:SetChecked(nil)
	elseif num == 103 then 
		LPNCONFIG.WHITE_TAILORING = 2
		if not checked then LPNCONFIG.WHITE_TAILORING = nil end
		LazyPigMenuObjects[102]:SetChecked(nil)
		LazyPigMenuObjects[104]:SetChecked(nil)
	elseif num == 104 then
		LPNCONFIG.WHITE_TAILORING = 0
		if not checked then LPNCONFIG.WHITE_TAILORING = nil end
		LazyPigMenuObjects[102]:SetChecked(nil)
		LazyPigMenuObjects[103]:SetChecked(nil)
	elseif num == 105 then
		LPNCONFIG.FOOD_AND_DRINK = 1
		if not checked then LPNCONFIG.FOOD_AND_DRINK = nil end
		LazyPigMenuObjects[106]:SetChecked(nil)
		LazyPigMenuObjects[107]:SetChecked(nil)
	elseif num == 106 then 
		LPNCONFIG.FOOD_AND_DRINK = 2
		if not checked then LPNCONFIG.FOOD_AND_DRINK = nil end
		LazyPigMenuObjects[105]:SetChecked(nil)
		LazyPigMenuObjects[107]:SetChecked(nil)
	elseif num == 107 then
		LPNCONFIG.FOOD_AND_DRINK = 0
		if not checked then LPNCONFIG.FOOD_AND_DRINK = nil end
		LazyPigMenuObjects[105]:SetChecked(nil)
		LazyPigMenuObjects[106]:SetChecked(nil)	
	elseif num == 108 then
		LPNCONFIG.ES_SHARDS = 1
		if not checked then LPNCONFIG.ES_SHARDS = nil end
		LazyPigMenuObjects[109]:SetChecked(nil)
		LazyPigMenuObjects[110]:SetChecked(nil)
	elseif num == 109 then 
		LPNCONFIG.ES_SHARDS = 2
		if not checked then LPNCONFIG.ES_SHARDS = nil end
		LazyPigMenuObjects[108]:SetChecked(nil)
		LazyPigMenuObjects[110]:SetChecked(nil)
	elseif num == 110 then
		LPNCONFIG.ES_SHARDS = 0
		if not checked then LPNCONFIG.ES_SHARDS = nil end
		LazyPigMenuObjects[108]:SetChecked(nil)
		LazyPigMenuObjects[109]:SetChecked(nil)	
	else
...
function LazyPig_ZoneCheck()
	...
	local leavechat = LPNCONFIG.WORLDRAID and LazyPig_Raid() or LPNCONFIG.WORLDDUNGEON and LazyPig_Dungeon() or LPNCONFIG.WORLDBG and LazyPig_BG() or LPNCONFIG.WORLDUNCHECK
	for i = 1, NUM_CHAT_WINDOWS do	
		...
		if ChatFrame:IsVisible() and not UnitIsDeadOrGhost("player") then
			...
			if (LPNCONFIG.WORLDRAID or LPNCONFIG.WORLDDUNGEON or LPNCONFIG.WORLDBG) and not leavechat then						
...
function LazyPig_CancelAspect()
	if LPNCONFIG.ASPECT == nil then return end
...
function LazyPig_CheckSalvation()
	if(LPNCONFIG.SALVA == 1 or LPNCONFIG.SALVA == 2 and (LazyPig_IsShieldEquipped() and LazyPig_PlayerClass("Warrior", "player") or LazyPig_IsBearForm())) then
...
function LazyPig_CheckManaBuffs()
	if(LPNCONFIG.REMOVEMANABUFFS == 1) then
...
function LazyPig_ChatFrame_OnEvent(event)
	if event == "CHAT_MSG_LOOT" or event == "CHAT_MSG_MONEY" then
		...
		local check_uncommon = LPNCONFIG.SPAM_UNCOMMON and string.find(arg1 ,"1eff00")
		local check_rare = LPNCONFIG.SPAM_RARE and string.find(arg1 ,"0070dd")
		local check_loot = LPNCONFIG.SPAM_LOOT and (string.find(arg1 ,"9d9d9d") or string.find(arg1 ,"ffffff") or string.find(arg1 ,"Your share of the loot"))
		local check_money = LPNCONFIG.SPAM_LOOT and string.find(arg1 ,"Your share of the loot")
	
		local check1 = string.find(arg1 ,"You")
		local check2 = string.find(arg1 ,"won") or string.find(arg1 ,"receive")
		local check3 = LPNCONFIG.AQ and (idol or scarab)
		local check4 = LPNCONFIG.ZG and (bijou or coin)
	...
	if LPNCONFIG.SPAM and arg2 and arg2 ~= GetUnitName("player") and (event == "CHAT_MSG_SAY" or event == "CHAT_MSG_CHANNEL" or event == "CHAT_MSG_YELL" or event == "CHAT_MSG_EMOTE" and not (IsGuildMate(arg2) or IsFriend(arg2))) then
		local time = GetTime()
